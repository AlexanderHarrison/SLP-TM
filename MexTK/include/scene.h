...
/*** Static Variables ***/
static SceneInfo *stc_scene_info = (void *)0x80479D30;
...
u8 Scene_GetCurrentMajor(void);
MajorSceneDesc *Scene_GetMajorSceneDesc(void);
MinorSceneDesc *Scene_GetMinorSceneDesc(void);
u8 Scene_GetCurrentMinor(void);
void Scene_SetNextMajor(int major_id); // run this in scene decide!
void Scene_ExitMajor(void);                // run this to cause a major scene change, usually ran in scene decide!
void Scene_SetNextMinor(int minor_id); // run this in scene decide!
void Scene_ExitMinor(void);                // run this to cause a minor scene change, usually ran in scene think!
void CSS_DecideNext(MinorScene *minor_scene, ScDataVS *css_data);
void CSS_ResetKOStars(void);
